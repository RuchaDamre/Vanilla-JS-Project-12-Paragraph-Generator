...
let paragraph = document.getElementById("para-container");
...
form.addEventListener('submit', (e) => {
  ...
  // Prevent the page from reloading after submitting form.
  e.preventDefault();
  ...
  if (num > 9) {
    for (let i = 0; i < (num - 9); i++) {
      ...
      para.push(para[random]);
    }
  }

  let tempText = para.slice(0, num)
  tempText = tempText
    .map(function (item) {
  ...
  //set array length to 9 so that it will not increase each time when we enter number greater than 9
  para.length = 9;
});
